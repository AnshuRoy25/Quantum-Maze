...
superposition_cooldown_time = 10  
last_tunnel_time = 0
last_blink_time = time.time()
blinking = False
blink_key_pressed = False  
blink_timer_frozen = False  
blink_disabled_until = 0  # To store the time when the key presses should be enabled again
...
def draw_maze():
    ...
    # Blinking cooldown timer (freezes at 0s until teleport)
    if blink_timer_frozen:
        blink_cd_text = font.render(f"Blink CD: 0s", True, PURPLE)
    else:
        blink_cd_text = font.render(f"Blink CD: {max(0, superposition_cooldown_time - int(time.time() - last_blink_time))}s", True, PURPLE)

    screen.blit(timer_text, (10, 10))
    screen.blit(prob_text, (10, 40))
    screen.blit(tunnel_cd_text, (10, 70))
    screen.blit(blink_cd_text, (10, 100))
...
def start_blinking():
    global blinking, last_blink_time, blink_key_pressed, blink_timer_frozen, blink_disabled_until

    if time.time() - last_blink_time >= superposition_cooldown_time:
        blinking = True
        last_blink_time = time.time()
        blink_key_pressed = False  
        blink_timer_frozen = True  # Freeze the cooldown timer at 0s
        blink_disabled_until = time.time() + 2  # Disable key presses for 2 seconds after blinking

def teleport_player():
    global player_x, player_y, blinking, blink_key_pressed, last_blink_time, blink_timer_frozen

    if blink_key_pressed:
        ...
        blinking = False  
        last_blink_time = time.time()  
        blink_timer_frozen = False  # Unfreeze the cooldown timer
...
while running:
    ...
    # Automatically trigger blinking every 10 seconds
    if time.time() - last_blink_time >= superposition_cooldown_time:
        start_blinking()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Block all key presses for 2 seconds after blinking starts
            if time.time() < blink_disabled_until:
                continue
            ...
            if blinking:
                blink_key_pressed = True  

    keys = pygame.key.get_pressed()
    if time.time() >= blink_disabled_until:  # Allow movement only after 2 seconds
        if keys[pygame.K_w]:
            move_player(0, -1)
        if keys[pygame.K_s]:
            move_player(0, 1)
    ...
    if blinking and blink_key_pressed:
        teleport_player()
